...
export default function Home() {
  ...
  const [subPanelHeight, setSubPanelHeight] = useState(40);
  ...
  const handleDragStart = (e: React.TouchEvent) => {
    ...
    dragStartH.current = subPanelHeight;
  };
  ...
  return (
    <div className={`flex h-[100dvh] overflow-hidden ${bg}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        {/* 고정 헤더 */}
        <header className={`sticky top-0 flex items-center justify-between px-3 py-2 border-b ${border} ${bg} z-20`}>
          <div className="flex items-center gap-2">
            ...
            {activeSub && !isWide && (
              <span className="text-[10px] text-[#f59e0b] bg-[#f59e0b]/10 px-1.5 py-0.5 rounded-full">팝업</span>
            )}
          </div>
          <div className="flex items-center gap-1">
            ...
            {!isWide && (
              <div className={`px-3 pb-3 pt-2 ${bg} flex-shrink-0`}>
                <div className={`chat-input-area ${popMode ? 'pop-mode' : ''} flex flex-col px-3 py-2 max-w-2xl mx-auto`}>
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#e5e5e5]/50">
                    <div className="flex items-center gap-3">
                      ...
                      <button className={`p-2 rounded-full ${dark ? 'bg-[#2a2a2a]' : 'bg-[#f0f0f0]'} ${text3}`}>
                        <Plus size={15} />
                      </button>
                    </div>
        ...
        {/* 데스크톱 입력 */}
        {isWide && (
          <div className={`px-3 pb-3 pt-1 ${bg} border-t ${border}`}>
            <div className="chat-input-area flex flex-col px-3 py-2 max-w-2xl mx-auto">
              <textarea value={input} onChange={(e) => setInput(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); if (input.trim()) { handleSendMain(input.trim()); setInput(''); } } }}
              ...
              <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#e5e5e5]/50">
                <div className="flex items-center gap-3">
                  {!activeSub && (
                    <button onClick={togglePopMode}
                      className={`p-2 rounded-full ${dark ? 'bg-[#2a2a2a]' : 'bg-[#f0f0f0]'} ${text3} hover:text-[#f59e0b]`}>
                      <Zap size={15} />
                    </button>
                  )}
                  <button className={`p-2 rounded-full ${dark ? 'bg-[#2a2a2a]' : 'bg-[#f0f0f0]'} ${text3}`}>
                ...
                <button onClick={() => { if (input.trim()) { handleSendMain(input.trim()); setInput(''); } }}
                  ...
                  className="p-2.5 bg-[#4a9eff] rounded-full disabled:opacity-30 select-none active:scale-95 transition-transform">
                  <Send size={15} className="text-white" />
                </button>
              </div>
            </div>
